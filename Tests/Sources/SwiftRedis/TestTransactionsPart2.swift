...
//  Phoenix
//
//  Created by Samuel Kallner on 06/01/2016.
...
public struct TestTransactionsPart2: XCTestCase {
        ...
        setupTests() {
            ...
            multi.exec() {(response: RedisResponse) in
                if  let nestedResponses = self.baseAsserts(response, count: 9)  {
                    ...
                    var newValue = NSData(bytes: bytes, length: bytes.count)
                    ...
                    newValue = NSData(bytes: bytes, length: bytes.count)
