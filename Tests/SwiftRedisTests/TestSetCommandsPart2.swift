...
public class TestSetCommandsPart2: XCTestCase {
    ...
    let redisKey1 = RedisString(Data(bytes: [0xff, 0x00, 0xfe, 0x02], count: [0xff, 0x00, 0xfe, 0x02].count))
    let redisKey2 = RedisString(Data(bytes: [0xfe, 0x01, 0xee, 0x03], count: [0xfe, 0x01, 0xee, 0x03].count))
    let redisKey3 = RedisString(Data(bytes: [0xfd, 0x10, 0xde, 0x04], count: [0xfd, 0x10, 0xde, 0x04].count))
    let redisKey4 = RedisString(Data(bytes: [0xfc, 0x11, 0xce, 0x05], count: [0xfc, 0x11, 0xce, 0x05].count))

    let member1 = "its over 9000"
    let member2 = "What up doc?"
    let member3 = "insert reference here"
    let member4 = "insert test string here"
    let redismember1 = RedisString(Data(bytes: [0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77], count: [0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77].count))
    let redismember2 = RedisString(Data(bytes: [0x12, 0x23, 0x34, 0x45, 0x56, 0x67, 0x78], count: [0x12, 0x23, 0x34, 0x45, 0x56, 0x67, 0x78].count))
    let redismember3 = RedisString(Data(bytes: [0x13, 0x24, 0x35, 0x46, 0x57, 0x68, 0x79], count: [0x13, 0x24, 0x35, 0x46, 0x57, 0x68, 0x79].count))
    let redismember4 = RedisString(Data(bytes: [0x14, 0x25, 0x36, 0x47, 0x58, 0x69, 0x70], count: [0x14, 0x25, 0x36, 0x47, 0x58, 0x69, 0x70].count))
